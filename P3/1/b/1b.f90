		...
		PROGRAM ex1
			IMPLICIT NONE
			INTEGER	::	time = 10000 ! Tempo total máximo (em segundos); uso um tempo grande o bastante para que a vel terminal seja atingida
			REAL*8	::	dt = 0.01 ! Intervalo de tempo delta t, pode ser alterado para 0.1
			...
			DO i= 1, n
				...
				IF (ABS(dt*(Pc - SQRT(2/(mass**3))*drag_coef*area*(K**(3.0/2.0)))) < 0.001) THEN ! Condição para velocidade terminal atingida: variação da energia cinética menor que E-3
					n = i
